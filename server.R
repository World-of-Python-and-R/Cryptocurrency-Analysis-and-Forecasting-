...
shinyServer(function(input, output){
    ...
    predictions <- reactive({
        ...
        data_b_prime <- diff(log(data_b))
        
        set.seed(123)
        model <- auto.arima(data_b_prime, lambda = "auto")
    ...
    output$fp <- renderText({
        ...
        means_prime <- xts(forecasts$mean, order.by = Sys.Date() + 1:julian(forecast_date, Sys.Date()))
        lower_prime <- xts(forecasts$lower[, 1], order.by = Sys.Date() + 1:julian(forecast_date, Sys.Date()))
        upper_prime <- xts(forecasts$upper[, 1], order.by = Sys.Date() + 1:julian(forecast_date, Sys.Date()))
        total_prime <- rbind.xts(data_b_prime, means_prime)
        total_prime <- merge(total_prime, lower_prime, upper_prime)
        
        total_prime[is.na(total_prime)] <- 0
        
        total_prime[[1, 1]] <- log(data_b[[1, 1]])
        
        for (j in 2:3) {
            for (k in 1:length(index(data_b))) {
                total_prime[[k, j]] <- total_prime[[k, 1]]
            }
        }
        
        for (j in 1:3) {
            for (i in 2:length(index(total_prime))) {
                total_prime[[i, j]] <- total_prime[[i, j]] + total_prime[[i-1, 1]]
            }  
        }
        
        for (j in 1:3) {
            for (i in 1:length(index(total_prime))) {
                total_prime[[i, j]] <- exp(total_prime[[i, j]])
            }  
        }
        
        isolate(total_prime[[length(index(total_prime)), 1]])
    ...
    output$chart4 <- renderPlot({
        ...
        plot(tail(total_prime, 365), main = "Forecasts")
    })
    ...
    output$tbl <- renderTable({
        ...
        dates <- as.character(index(total_prime))
        total_prime <- as.data.frame(total_prime)
        total_prime$date <- dates
        names(total_prime) <- c("Mean of Forecasts", "Lower Bound", "Upper Bound", "Date")
        print(tail(total_prime, julian(forecast_date, Sys.Date())))
    })
})